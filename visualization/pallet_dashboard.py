"""
3-D Palletization Visualization Dashboard
==========================================

A Dash web application that reads a palletization result CSV and renders
one card per pallet in a 2-column scrollable grid.

Usage:
    python visualization/pallet_dashboard.py results.csv

Layout:
    2-column grid — one card per pallet.
    Each card has:
        Top    — Pallet label (ID + client info)
        Left   — Interactive 3-D pallet view (rotate / zoom / pan)
        Right  — Statistics panel
        Bottom — Per-pallet client/priority legend
"""
...
PAGE_ZOOM = 1.0  # Zoom CSS désactivé : à <1 Plotly décale les tooltips car il
                 # calcule le hover sur les coordonnées non-scalées.
...
def load_pallet_data(csv_path: str) -> pd.DataFrame:
    ...
    print(f"[Dashboard] Loaded {len(df)} placed boxes across "
...
def _render_slot_2d(pallet_id, df: pd.DataFrame, color_map: dict, view: str) -> go.Figure:
    ...
    fig.update_layout(
        ...
        annotations=[dict(
            ...
            showarrow=False,
            font=dict(size=18, family="Arial Black", color="#000"),
        )],
    )
    return fig
...
def _render_slot(pallet_id, df: pd.DataFrame, color_map: dict,
    ...
    stat_rows = [
        html.H4(
            f"Palette {pallet_id} — Statistiques",
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.Div(id=f"stats-{slot}",
                             style={"width": "285px", "flexShrink": "0", "paddingTop": "4px"}),
                ]
            ),
...
def _dashboard_layout(pallet_ids: list, dropdown_options: list,
                      n_pages: int, logo_b64: str, logo2_b64: str = "", csv_name: str = "") -> html.Div:
    btn_style = {
    ...
    pagination = html.Div(
        ...
        children=[
            html.Button("◀  Préc.", id="btn-prev", n_clicks=0, style=btn_style),
    ...
    pagination_bottom = html.Div(
        style={"display": "flex", "justifyContent": "center",
               "alignItems": "center", "gap": "16px", "marginTop": "16px"},
        children=[
            html.Button("◀  Préc.", id="btn-prev-bottom", n_clicks=0, style=btn_style),
...
def _build_p2_table(df_p: pd.DataFrame, highlight_seq: int = None) -> html.Div:
    ...
    if df_p2.empty:
        return html.Div(
            "Aucun colis P2 sur cette palette.",
    ...
    th_style = {
        ...
        "fontSize": "16.5px", "color": "#888",
        "borderBottom": "1px solid #ddd",
        "position": "sticky", "top": "0", "backgroundColor": "#fafafa",
    }
    ...
    for _, row in df_p2.iterrows():
        ...
        rows.append(html.Tr(
            ...
            children=[
                html.Td(loc,              style={**td_base, "fontWeight": fw}),
    ...
    return html.Div(
        ...
        children=[
            html.H5("Colis P2",
                    style={"margin": "0 0 8px 0", "color": "#e67e22",
...
def _zoom_layout(dropdown_options: list, logo_b64: str, logo2_b64: str = "", default_pid=None, csv_name: str = "") -> html.Div:
    return html.Div(
        style={"fontFamily": "Arial, sans-serif", "backgroundColor": "#f5f5f5",
               "minHeight": "100vh", "padding": "16px"},
        children=[
            ...
            html.Div(
                ...
                children=[
                    dcc.Graph(
                        id="zoom-graph",
                        style={"flex": "1", "minWidth": "0", "height": "720px"},
                        config={"displayModeBar": True, "scrollZoom": True,
                                "modeBarButtonsToRemove": ["select2d", "lasso2d"]},
                    ),
                    html.Div(
                        style={"width": "590px", "flexShrink": "0", "paddingTop": "4px",
                               "display": "flex", "flexDirection": "column", "gap": "0px"},
                        children=[
                            html.Div(id="zoom-stats"),
                            html.Div(id="zoom-p2-table"),
                        ],
                    ),
                ]
            ),
            ...
            html.Div(id="zoom-legend",
                     style={"fontSize": "15px", "color": "#666", "marginTop": "8px"}),
            html.Div(id="zoom-box-label",
                     style={"fontSize": "26px", "fontWeight": "bold", "color": "#222",
                            "padding": "8px 0", "minHeight": "36px"}),
        ]
    )
...
def _register_zoom_callbacks(app, df, color_map, pallet_ids):
    ...
    def update_zoom_figure(seq_max, pid):
        ...
        seq  = int(seq_max)
        df_p = df[df["pallet_id"] == pid]
        fig, _, _, box_label = _render_slot(
            pid, df, color_map,
            seq_max=seq,
            highlight_seq=seq,
        )
        is_p2_step = not df_p[
            (df_p["sequence"] == seq) & (df_p["priority"] == 2)
        ].empty
...
def build_app(df: pd.DataFrame, csv_path: str = "") -> dash.Dash:
    """
    Builds a single multi-page Dash app with Dashboard and Zoom views.

    URL routing:
        /     → paginated slot dashboard
        /zoom → single-pallet zoom view with placement replay slider
    """
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python visualization/pallet_dashboard.py <results.csv>")
        sys.exit(1)
    ...
    host = _os.environ.get("PALLET_HOST", "127.0.0.1")
    port = int(_os.environ.get("PALLET_PORT", "8051"))

    base = f"http://{host}:{port}"
    print(f"\n[Dashboard] Starting server at {base}")
    print(f"[Dashboard] Opening:  {base}/  |  {base}/zoom")
    ...
    if host == "127.0.0.1":
        ...
        _ts = int(_time.time())
        def _open_tabs():
            webbrowser.open(f"{base}/?_={_ts}")
            webbrowser.open(f"{base}/zoom?_={_ts}")
        Timer(1.5, _open_tabs).start()
